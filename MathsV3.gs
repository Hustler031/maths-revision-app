...
function toggleImportantV3(questionId,sessionId){const id=validQuestionId_(questionId),cur=getStateMap_()[id]||{},next=!isMarked_(cur),st=upsertState_(id,{marked:next});logMathsV3Flag_(id,'IMPORTANT',next,sessionId);return {ok:true,questionId:id,important:!!st.marked,difficult:isDifficultV3_(getStateMap_()[id])};}
function toggleDifficultV3(questionId,sessionId){
  const id=validQuestionId_(questionId);ensureMathsV3_();const sh=getSheet_(MATHS.SHEETS.STATE),headers=sh.getRange(1,1,1,sh.getLastColumn()).getValues()[0].map(key_),col=headers.indexOf('difficult')+1;
  let rows=sheetObjects_(sh).filter(r=>String(r.question_id||'').trim()===id),current=rows.some(r=>isDifficultV3_(r)),next=!current;
  if(!rows.length){upsertState_(id,{});rows=sheetObjects_(sh).filter(r=>String(r.question_id||'').trim()===id);}
  rows.forEach(r=>sh.getRange(r.__row,col).setValue(next));logMathsV3Flag_(id,'DIFFICULT',next,sessionId);
...
function startMathsV3Quiz(request){
  ensureMathsV3_();request=request||{};const state=getStateMap_(),kind=String(request.kind||'random').toLowerCase(),count=Math.max(1,Math.min(100,Number(request.count||20)));
  if(kind==='all'&&!request.restart){const found=findV3AllSession_(request);if(found)return enhanceV3Payload_(resumeSession(String(found.session_id)),state);}
  let pool=scopePoolMathsV3_(request),label='Practice';
  ...
  else if(kind==='difficult'){pool=pool.filter(q=>isDifficultV3_(state[q.question_id]));label='Difficult';}
  else if(kind==='weak'){
    pool=pool.filter(q=>!isMastered_(state[q.question_id])).map(q=>{const s=state[q.question_id]||{},wrong=String(s.last_result||'').toLowerCase()==='wrong',slow=Number(s.last_response_sec||0)>=20,diff=isDifficultV3_(s),attempts=Number(s.attempts||0);return {q,score:(diff?100:0)+(wrong?60:0)+(slow?30:0)+Math.min(attempts,10)*3+(attempts===0?5:0)}}).sort((a,b)=>b.score-a.score).map(x=>x.q);label='Weak';
  ...
  if(kind!=='all'&&kind!=='weak')pool=shuffle_(pool);if(kind!=='all')pool=pool.slice(0,Math.min(count,pool.length));
  if(!pool.length)return {ok:false,message:'No eligible '+label.toLowerCase()+' questions found for this selection.'};
  const titleBase=String(request.title||request.majorTopic||request.chapter||request.groupName||'Maths'),title=titleBase+' · '+label,sessionId=Utilities.getUuid(),mode='v3_'+kind,payload=enhanceV3Payload_(sessionPayload_(sessionId,pool,state,title,mode,0,null),state);
...
function getSessionAttemptMapV3(sessionId){const out={};sheetObjects_(getSheet_(MATHS.SHEETS.ATTEMPTS)).filter(r=>String(r.session_id||'')===String(sessionId||'')).forEach(r=>{const id=String(r.question_id||'');if(!id)return;const v=String(r.result||'').toLowerCase();out[id]=v==='correct'?'correct':v==='wrong'?'wrong':'seen';});return out;}
