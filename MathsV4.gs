...
function getStarredRevisionV4(){
  ...
  const ids=Object.keys(membership),currentDay=mathsV3Day_();
  ...
  const blocks=[];for(let hi=Math.floor((currentDay-1)/10)*10+10;hi>=1;hi-=10){const lo=Math.max(1,hi-9),members={};dayRows.filter(x=>x.day>=lo&&x.day<=hi).forEach(x=>Object.keys(days[x.day]||{}).forEach(id=>members[id]=true));if(Object.keys(members).length)blocks.push({lo,hi,stats:statsFor(members),days:dayRows.filter(x=>x.day>=lo&&x.day<=hi)});}
