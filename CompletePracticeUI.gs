...
function completePracticeUiPatch_(){
  ...
  const newCheck="function delayRefresh(ms){return new Promise(r=>setTimeout(r,ms))}function refreshHomeSilent(){return call('home').then(d=>{if(String(d&&d.eligibilityVersion||'')!==ACADEMIC_COMPAT)throw new Error('Incompatible Maths home');write('home',d);if(app.view==='home')renderHome(d);return d}).catch(()=>null)}function refreshBootstrapSilent(){return refreshBootstrap().catch(()=>null)}function reconcileCaches(nv){knownVersion=String(nv||knownVersion);return refreshHomeSilent().then(()=>refreshBootstrapSilent()).then(()=>delayRefresh(80)).then(()=>prefetch())}function checkVersion(){return call('version').then(v=>{const nv=String(v&&v.version||'');if(!nv)return;const changed=!!knownVersion&&nv!==knownVersion;if(changed)return reconcileCaches(nv);knownVersion=nv}).catch(()=>{})}";
