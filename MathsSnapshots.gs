...
function buildMathsSnapshotV6_(){
  ...
  return {generatedAt:new Date().toISOString(),overall:metric(all),advanced:metric(all.filter(q=>mathsV3Group_(q.chapter)==='advanced')),arithmetic:metric(all.filter(q=>mathsV3Group_(q.chapter)==='arithmetic')),chapters};
}
...
function getMathsNewHubV6(){const snap=getMathsSnapshotV6(false);return {generatedAt:snap.generatedAt,overall:snap.overall,chapters:(snap.chapters||[]).filter(x=>Number(x.metric&&x.metric.unseen||0)>0)};}
function cachedHubV6_(name,builder,ttl){const c=mathsSnapshotCache_(),k=mathsSnapshotKey_(name),raw=c.get(k);if(raw){try{return JSON.parse(raw)}catch(e){}}const v=builder();try{c.put(k,JSON.stringify(v),ttl||MATHS_HUB_CACHE_SEC)}catch(e){}return v;}
...
function invalidateMathsSnapshotsV6(){const c=mathsSnapshotCache_();['progress','starred','concepts'].forEach(k=>{try{c.remove(mathsSnapshotKey_(k))}catch(e){}});return true;}
function getMathsScopeMetricV6(request){ensureMathsV3_();const m=mathsV4Metric_(mathsV5Pool_(request||{}),getStateMap_()),attempted=Math.max(0,Number(m.attempted||0));return Object.assign({},m,{wrongPct:mathsPct_(m.wrong,attempted),difficultPct:mathsPct_(m.difficult,attempted),starredPct:mathsPct_(m.starred,attempted)});}
...
/* Keep the expensive Progress snapshot on its hourly TTL. Starred/Concept hubs can refresh independently. */
function toggleStarredV6(questionId,sessionId){const r=toggleImportantV3(questionId,sessionId);try{mathsSnapshotCache_().remove(mathsSnapshotKey_('starred'))}catch(e){}return r;}
function toggleDifficultV6(questionId,sessionId){return toggleDifficultV3(questionId,sessionId);}
...
function startMathsV6Quiz(request){
  ...
  /* Practice All means the complete active pool, including historically mastered questions. */
