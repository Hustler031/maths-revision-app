...
function getResumeSession_() {
  ...
  if (!rows.length) return null; const s = rows[0]; const ids = json_(s.question_ids_json, []);
  return {sessionId:s.session_id,title:s.title,currentIndex:Number(s.current_index || 0),total:ids.length,mode:s.mode};
}

function isCalculativeDemandQuestion_(q) {
  const hay=normalizeLabel_([q.card_type,q.topic,q.subtopic,q.prompt].join(' '));
  if(/concept|theorem|definition|property|formula recall|memory/.test(normalizeLabel_(q.card_type))) return false;
  return /calculat|numerical|arithmetic|find|compute|value|percentage|profit|loss|discount|ratio|proportion|average|interest|time|work|speed|distance|mixture|alligation|number|mensuration/.test(hay);
}

function buildDemandSetMeta_(ids) {
  const map={}; getAllQuestions_().concat(getGeneratedQuestions_()).forEach(q=>map[String(q.question_id)]=q);
  const questions=(ids||[]).map(id=>map[String(id)]).filter(q=>q&&active_(q));
  const chapterMap={}; let calculative=0;
  questions.forEach(q=>{
    const chapter=String(q.chapter||'Other').trim()||'Other';
    if(!chapterMap[chapter])chapterMap[chapter]={name:chapter,count:0};
    chapterMap[chapter].count++;
    if(isCalculativeDemandQuestion_(q))calculative++;
  });
  return {organized:true,calculativeCount:calculative,chapters:Object.values(chapterMap).sort((a,b)=>a.name.localeCompare(b.name))};
}

function getDemandSets_() {
  return sheetObjects_(getSheet_(MATHS.SHEETS.DEMAND_SETS)).filter(r => r.set_id && String(r.status || 'Active').toLowerCase() !== 'inactive').map(r => {
    const ids = json_(r.question_ids_json, []);
    const base = {setId:String(r.set_id),name:String(r.set_name || 'Demand Set'),description:String(r.description || ''),count:ids.length,createdAt:r.created_at ? Utilities.formatDate(new Date(r.created_at), studyTimezone_(), 'dd MMM yyyy') : ''};
    if (base.setId === MATHS.CALC_SET_ID) {
      const groups = calculationDayGroups_(ids); const keys = Object.keys(groups).sort(); base.dayCount = keys.length;
      base.days = keys.map((key, idx) => ({key:key, dayNumber:idx+1, dateLabel:formatCalcDate_(key), count:groups[key].length}));
    } else Object.assign(base,buildDemandSetMeta_(ids));
    return base;
  });
}

function getDemandSetById_(id) { return sheetObjects_(getSheet_(MATHS.SHEETS.DEMAND_SETS)).find(r => String(r.set_id) === String(id) && String(r.status || 'Active').toLowerCase() !== 'inactive'); }
...
function shuffle_(a) { a = a.slice(); for (let i=a.length-1;i>0;i--) { const j = Math.floor(Math.random() * (i+1)); const t = a[i]; a[i] = a[j]; a[j] = t; } return a; }
